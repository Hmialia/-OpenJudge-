...
ElemType* Findpirents(BTNode *bt,ElemType x){
	if(bt==NULL||(bt->lchild==NULL&&bt->rchild==NULL)){
		return NULL;
	}
	else if(bt->lchild==NULL&&bt->rchild!=NULL){
		if(bt->rchild->data==x) return &(bt->data);
		else return Findpirents(bt->rchild,x);
	}
	else if(bt->lchild!=NULL&&bt->rchild==NULL){
		if(bt->lchild->data==x) return &(bt->data);
		else return Findpirents(bt->lchild,x);
	}
	else {
		if(bt->lchild->data==x||bt->rchild->data==x){
			...
			ElemType* lp=Findpirents(bt->lchild,x);
			ElemType* rp=Findpirents(bt->rchild,x);
			if(lp!=NULL) return lp;
			else if(rp!=NULL) return rp;
			else return NULL;
		}
	}
}
...
int main()
{
	...
	if(Findpirents(bt,x)==NULL){
		cout<<"NULL\n";
	}
	
	else {
		ElemType c=*Findpirents(bt,x);
		cout<<c<<endl;
	}
